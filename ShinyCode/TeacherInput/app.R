...
urlfile <- "https://raw.githubusercontent.com/ndavis4904/conners_score/main/Data/Conners_Teacher_Updated.csv"
teacher <- read.csv(url(urlfile))
...
ui <- fluidPage(
    ...
    sidebarLayout(
        ...
        tabPanel("T-Scores",
            ...
            h3("Learning Problems/Executive Functioning"),
            textOutput("LE"),
            
            h4("Learning Problems"),
            textOutput("LP"),
            
            h4("Executive Functioning"),
            textOutput("EF"),
...
server <- function(input, output) {
    ...
    output$diff_phrase <- renderText(paste0("Note: If the total number of differences of 2s and 3s (seen above in the graph) are greater than 2,",
                                            "<b>"," AND ", "</b>", sum(c(abs(diff(c(input$Q97, input$Q100))), abs(diff(c(input$Q52, input$Q63))), 
                                                                  ...
                                                                  abs(diff(c(input$Q25, input$Q57))), abs(diff(c(input$Q23, input$Q44))),
                                                                  abs(diff(c(input$Q34, input$Q89))), abs(diff(c(input$Q47, input$Q71))))),
                                              " is greater than 6, there may be a question of consistency in responses."))
    
    #Calculating positive response style
    output$PI_1 <- renderText(paste0("Total Positive Impressions is ", sum(ifelse(input$Q15 == 3, 1, 0),
                                                                         ifelse(input$Q28 == 3, 1, 0),
                                                                         ifelse(input$Q30 == 0, 1, 0),
                                                                         ifelse(input$Q36 == 0, 1, 0),
                                                                         ifelse(input$Q55 == 3, 1, 0),
                                                                         ifelse(input$Q109 == 3, 1, 0)), "."))
    output$PI_2 <- renderText(paste0("If ", "<b>", sum(ifelse(input$Q15 == 3, 1, 0),
                                               ifelse(input$Q28 == 3, 1, 0),
                                               ifelse(input$Q30 == 0, 1, 0),
                                               ifelse(input$Q36 == 0, 1, 0),
                                               ifelse(input$Q55 == 3, 1, 0),
                                               ifelse(input$Q109 == 3, 1, 0)), "</b>",
                                    " is greater than 5, there is a possible positive response style indicated."))
    
    #Calculating negative response style
    output$NI_1 <- renderText(paste0("Total Negative Impressions is ", sum(ifelse(input$Q5 > 1, 1, 0),
                                                                           ifelse(input$Q81 < 2, 1, 0),
                                                                           ifelse(input$Q93 > 1, 1, 0),
                                                                           ifelse(input$Q101 > 1, 1, 0),
                                                                           ifelse(input$Q107 < 2, 1, 0),
                                                                           ifelse(input$Q110 > 1, 1, 0)), "."))
    output$NI_2 <- renderText(HTML(paste0("If ", "<b>", sum(ifelse(input$Q5 > 1, 1, 0),
                                                ifelse(input$Q81 < 2, 1, 0),
                                                ifelse(input$Q93 > 1, 1, 0),
                                                ifelse(input$Q101 > 1, 1, 0),
                                                ifelse(input$Q107 < 2, 1, 0),
                                                ifelse(input$Q110 > 1, 1, 0)), "</b>",
                                     " is greater than 5, there is a possible positive response style indicated.")))
